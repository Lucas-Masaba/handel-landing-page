...
const Footer2 = () => {
  ...
  return (
    <div className="bg-gray-800 dark:bg-gray-800 w-full">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        {columns.map(({ title, texts }, index) => (
          ...
          >
            <div className="mb-2 font-medium text-gray-300 dark:text-gray-300">{title}</div>
            {texts &&
              texts.map((text, index2) => (
                <p key={`item-text-${index2}`} className="text-slate-400 dark:text-slate-400">
                  {text}
                </p>
              ))}
          </div>
        ...
        <div className="col-span-4 sm:col-span-2 md:col-span-2 lg:col-span-1 xl:col-span-1">
          <div className="mb-2 font-medium text-gray-300 dark:text-gray-300">Social</div>
      ...
      <div className="text-muted py-6 text-sm text-slate-400 dark:text-slate-400 md:flex md:items-center md:justify-between md:py-8">
        <ul className="mb-4 flex pl-2 rtl:pl-0 rtl:pr-2 md:order-1 md:mb-0">
          {links &&
            links.map(({ label, href }, index) => (
              <li key={`item-link-${index}`}>
                <a
                  className="duration-150 ease-in-out text-gray-400 placeholder:transition hover:text-[var(--brand-accent-500)] hover:underline dark:text-gray-400"
                  aria-label={label}
                  href={href}
                >
                  {label}
